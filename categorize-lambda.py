...
def predict(input_doc):
    ...
    confidence = str(model.predict_proba(test_features).max()*100)+'%'

    return prediction.tolist()[0],confidence
...
def lambda_handler(event, context):
    ...
    response = table.update_item(
        Key={'id':postId},
          UpdateExpression=
            "SET #statusAtt = :statusValue, #classAtt = :classValue, #confidenceAtt = :confidenceValue",                  
          ExpressionAttributeValues=
            {':statusValue': 'UPDATED', ':classValue': result, ':confidenceValue': confidence},
        ExpressionAttributeNames=
          {'#statusAtt': 'status', '#classAtt': 'class', '#confidenceAtt': 'confidence'},
    )
